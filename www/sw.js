// ═══════════════════════════════════════════════════════════════════
// SERVICE WORKER (FCM + Appels) — VERSION CHEMINS RELATIFS
// Marche sur n'importe quelle URL (avec ou sans sous-dossier)
// ═══════════════════════════════════════════════════════════════════
...
// ⚠️ Dériver le base path automatiquement depuis l'URL du SW
// Ex : si SW = https://x.github.io/MON-DEPOT/sw.js → BASE = /MON-DEPOT/
const SW_PATH = self.location.pathname;
const BASE_PATH = SW_PATH.substring(0, SW_PATH.lastIndexOf('/') + 1);

console.log('[SW] ✅ Service Worker chargé v4');
console.log('[SW] BASE_PATH détecté:', BASE_PATH);
...
self.addEventListener('notificationclick', function(event) {
  ...
  // ⚠️ URL relative au base path
  const url = BASE_PATH + 'index.html?call=' + callId + '&autoanswer=1';
